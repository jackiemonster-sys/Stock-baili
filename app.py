...
def find_recent_two_weeks_divergence(df):
    """
    限定只檢測【近 2 週 (約 10 個交易日)】內出現的背離現象
    透過比對近 5 天極值與前 5~15 天的極值
    """
    if df is None or len(df) < 20:
        return {}

    # 取近 2 週 (20 個交易日) 進行細分比對
    recent_2w = df.iloc[-20:]
    
    # 拆分為最近 5 天 (當前) 與 前 5~15 天 (對照組)
    recent = recent_2w.iloc[-5:]
    past = recent_2w.iloc[-15:-5]
    ...
    for ind in indicators:
        ...
        # 價格極值與日期
        p_low_past, p_low_rec = past['Low'].min(), recent['Low'].min()
        p_high_past, p_high_rec = past['High'].max(), recent['High'].max()

        # 指標極值
        i_low_past, i_low_rec = past[ind].min(), recent[ind].min()
        i_high_past, i_high_rec = past[ind].max(), recent[ind].max()

        # 1. 底背離：近 5 天價格低於前一段，但指標未破低
        if p_low_rec < p_low_past and i_low_rec > i_low_past:
            start_dt = past['Low'].idxmin().strftime('%Y-%m-%d')
            end_dt = recent['Low'].idxmin().strftime('%Y-%m-%d')
            results[bull_key].append({
                'start_date': start_dt,
                'end_date': end_dt,
                'start_price': float(p_low_past),
                'end_price': float(p_low_rec)
            })

        # 2. 頂背離：近 5 天價格高於前一段，但指標未創新高
        if p_high_rec > p_high_past and i_high_rec < i_high_past:
            start_dt = past['High'].idxmax().strftime('%Y-%m-%d')
            end_dt = recent['High'].idxmax().strftime('%Y-%m-%d')
            results[bear_key].append({
                'start_date': start_dt,
                'end_date': end_dt,
                'start_price': float(p_high_past),
                'end_price': float(p_high_rec)
            })
...
# 側邊欄：背離條件獨立/共同勾選
# ==========================================
st.sidebar.header("🔍 背離條件選擇 (限定近二週)")
...
if selected_conditions:
    with st.spinner("正在掃描近二週出現背離的股票..."):
        for symbol, name in DEFAULT_STOCKS.items():
            df = get_stock_data(symbol)
            if df is not None:
                divs = find_recent_two_weeks_divergence(df)
...
if not selected_conditions:
    ...
    st.warning("⚠️ 近二週內沒有符合勾選背離條件的股票。可嘗試切換為「滿足任一條件 (OR)」或勾選其他指標！")
else:
    st.subheader(f"🎯 近二週出現目標背離的股票 (共 {len(matched_stocks)} 檔)")
    ...
    if selected_stock:
        ...
        with tab5:
            ...
            if has_bull:
                st.success(f"**短線看多 / 止跌反彈機會**\n近二週出現【{', '.join(details.keys())}】，短線價格探底但技術指標止跌，有機會迎來反彈攻勢，向上觀察 20MA ({ma20:.2f} 元) 壓力。")
            else:
                st.error(f"**短線看空 / 拉回修正風險**\n近二週出現【{', '.join(details.keys())}】，股價高位衝高但指標未能同步，推升動能不足，注意短線回測 20MA ({ma20:.2f} 元) 的風險。")
